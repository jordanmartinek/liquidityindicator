...
### ⑭ Confluence & Context
- **MTF liquidity dashboard** — a compact table pinned to the middle-right showing, per timeframe (default 15m / 1H / 4H / D): nearest buy-side liquidity above, nearest sell-side below, directional bias **with a conviction %** (EMA separation vs recent range — read higher-timeframe conviction at a glance), premium/discount location, and the **target price** (the liquidity level in the direction of that timeframe's bias).
